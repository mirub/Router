...
arp_table_entry *has_arp_entry(std::vector<arp_table_entry> arp_table, arp_table_entry entry) {

	for (int i = 0; i <= arp_table.size(); ++i) {
		if (arp_table[i].ip == entry.ip && arp_table[i].mac == entry.mac) {
			return &arp_table[i];
		}
	}
	return nullptr;
...
int main(int argc, char *argv[]) {
	...
	while (1) {
		...
		std::cout <<" MUIE PC "<<std::endl; // STERGE ASTAA
	}
